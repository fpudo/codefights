...
// You can't let this ruin your reputation, so you want to apply the box
...
// of the pixel values from the 3 × 3 square that has its center at x, i
// ncluding x itself. All the pixels on the border of x are then removed.
//
...
function boxBlur(img) {
    let imgArr = [];
    for(y = 0; y < img.length - 2; y++){
        let line = [];
        for(x = 0; x < img[y].length -2; x++){
            let sum = 0;
            let count = 0;
            for(a = y; a < y + 3; a++){
                for(b = x; b < x +3; b++){
